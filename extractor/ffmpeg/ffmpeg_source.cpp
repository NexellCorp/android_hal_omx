...
int64_t FFSource::seek(int64_t pos, int whence)
{
	if( whence == SEEK_CUR )
	{
		mOffset += pos;
...
static int android_open(URLContext *h, const char *url, int /*flags*/)
{
	...
	sscanf(url + strlen("android-source:"), "%x", (int*)&source);
	if(source == NULL){
...
//extern "C" int ffurl_register_protocol( void *protocol, int size);
