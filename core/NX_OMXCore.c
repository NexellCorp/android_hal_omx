...
static OMX_ERRORTYPE RegisterAllComponents()
{
	...
	NX_OMXRegisterComponent("OMX.NX.AUDIO_DECODER.FFMPEG.ape",		"libNX_OMX_AUDIO_DECODER_FFMPEG",	"audio_decoder.ape",		NULL);
#endif
...
//
//	TODO : NX_OMX_ComponentNameEnum should be modified by judging
//	whether to use Registered information or actual information.
//
	...
	OMX_IN  OMX_CALLBACKTYPE* pCallBacks)
{
	...
	if( eError == OMX_ErrorNone ){
		*pHandle = (OMX_HANDLETYPE*)pComponent;
		...
		NxFree( pComponent );
		NX_LOGE("\t Error : ComponentInit() Failed, (%s, eError=%d(0x%08x) ).\n", cComponentName, eError, eError );
	}
    NX_LOGV("\t %s()--\n", __FUNCTION__);
	pthread_mutex_unlock( &gstOMXCoreMutex );
	return eError;
